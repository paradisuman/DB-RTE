...
class LockManager {
	...
	enum class GroupLockMode { NON_LOCK,
														 IS,
														 IX,
														 S,
														 X,
	...
	public:
		...
		txn_id_t first_hold_shared{-1};                          // 事务号最小，即当前事务更先执行，所以需要abort
	};
...
private:
	...
	std::unordered_set<txn_id_t> abort_set;
};
